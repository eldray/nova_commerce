import { schema, OutputType } from "./store_update_POST.schema";
...
export async function handle(request: Request) {
  try {
    ...
    if (input.brandColor !== undefined) {
      updateData.brandColor = input.brandColor || null;
    }
    if (input.tagline !== undefined) {
      updateData.tagline = input.tagline || null;
    }
    ...
    console.error("store_update error:", error);
